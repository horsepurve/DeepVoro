...
emb_2d = False # False | True # for MultiDigitMNIST: doesn't work
class WideResNet(nn.Module):
    def __init__(self, 
        ...
        if loss_type == 'softmax':
            ...
            if emb_2d:
                self.emb_2d = nn.Linear(nChannels[3], 2)
                self.linear = backbone.distLinear(2, int(num_classes))
            else:            
                self.classifier = backbone.distLinear(nChannels[3], int(num_classes)) # classifier | linear
    ...
    def forward(self, x, target= None, mixup=False, mixup_hidden=True, mixup_alpha=None , lam = 0.4):
        if target is not None: 
            ...
            if emb_2d:
                out = self.emb_2d(out)                
            out1 = self.classifier(out) # classifier | linear
