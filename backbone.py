...
with open('extra_params.json',"r") as f:
    eparam = json.load(f)
EMB = eparam['EMB'] # False True

class ConvNetS(nn.Module): #For omniglot, only 1 input channel, output dim is 64
    def __init__(self, depth, dropRate=0, flatten = True):
        ...
        self.trunk = nn.Sequential(*trunk)
        if EMB:
            self.final_feat_dim = 2 # 1024
            self.emb = nn.Linear(1024,2)
            # self.relu = nn.ReLU(inplace=True)
        else:
...
def Conv4S(dropRate=0):
    return ConvNetS(4,dropRate=dropRate)
